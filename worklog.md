...
Task ID: 39
Agent: Z.ai Code (glavni agent)
Task: P4-B zaključek motorja zvestobe — PAMETNA rojstnodnevna sporočila (+ popravek deploy postopka in izgubljenih demo fotografij)

Work Log:
- KONTROLA: Taski 37 in 38 že dokončani (worklog + produkcija live, tema „light"); nadaljevanje na zadnjem stebru P4 (B: rojstnodnevna sporočila — A win-back in C rebooking sta že iz Taska 38)
- AUDIT obstoječe kartice: čestitka generična s HARDCODIRANO obljubo „−20 %" (poslovna tveganost — salon morda ne nudi popusta), BREZ sledenja poslanih čestitk (nevarnost dvojnih sporočil — enak razred napake kot spomniki, Task 37 #15, tam popravljeno, tukaj spregledano), brez osebnosti, brez gumba Naroči, soonLabel ročna množina („3 strank" namesto „3 stranke"), demo rojstni dnevi vsi septembra (kartica prazna ob drugem letnem času), parseBirthdayInput sprejme nemogoče datume („31. 2.")
- API /api/birthdays NADGRAJEN: priljubljena storitev stranke (najpogostejša med zaključenimi obiski, izenačeno → novejša), dateKey prihajajoče ponovitve (ključ oznak + predlog termina), „next" tudi čez 45-dnevno okno (prazna kartica vseeno pove, kdaj je naslednja)
- birthday-card.tsx PREPISANA: oznake „poslano" (localStorage, ključ vsebuje LETO → samodejna ponastavitev naslednje leto, prune lanskoletnih, „Ponastavi oznake"), osebno sporočilo (priljubljena storitev + podpis salona, ton „ti" skladen z win-back), NASTAVLJIVA ugodnost (zobnik → urejevalnik, privzeto PRAZNO — brez obljub, ki jih lastnik ni odobril; shrani v localStorage), gumb NAROI (predizpolnjen vnos: stranka + njena storitev + dan rojstnega dne), slCount za dvojine/množine, refreshKey osveževanje
- POPRAVKI ob cesti: parseBirthdayInput mesečna validacija dni, dashboard poveže BirthdayCard z openManual + refreshKey
- SEED scripts/spread-birthdays.ts (obe bazi): septembrski sklop (Ana 09-05 danes, Petra 09-08, Maja 09-14, Marko 09-30 — živa predstavitev) + Luka 11-21, Nuša 01-17, Tina 05-23 (razpored čez leto, kartica nikoli prazna)
- E2E LOKALNO (agent-browser): API vrne Ana (inDays 0, priljubljena „Striženje — ženske") + next Luka (77 dni) ✓; kartica: „3 stranke v 14 dneh" (pravilna dvojina), Ana „danes 🎉", vrstica „najpogosteje: …", naslednja Marko ✓; WhatsApp sporočilo: „Živjo Ana! Vse najboljše… Kdaj ti ustreza termin za striženje — ženske? Lep pozdrav, Studio Aura" (brez popusta) ✓; urejevalnik ugodnosti: shranjen „−20 % na obisk" → sporočilo vključi „V rojstnem mesecu te pri nas čaka −20 % na obisk." ✓; klik WhatsApp → oznaka „poslano" + „1 čestitka že odprtih" + „Ponastavi oznake" (deluje) + localStorage pravilen ✓; NAROI za Ano → dialog SOB 5 sep IZBRAN + storitev + ime/telefon; za Petro → TOR 8 sep IZBRAN ✓; prazno stanje (test s premikom datumov + restavriranjem): „V naslednjih 45 dneh ni rojstnih dni — prva je Luka (21. nov, čez 77 dni)." ✓; tema LIGHT pod set media dark (html class light, color-scheme light, kremna podlaga) ✓; mobil 390px: sw=cw=390 (0 preliva), noga na dnu dokumenta ✓; 0 konzolnih napak, 0 page error; VLM desktop 9/10 + mobil 9/10; lint čist
- DEPLOY ODLOMEK (pomembna izkušnja): prvi poskusi so padli na „Module not found: Can't resolve 'tw-animate-css'" — VZROK: pri pripravi sourceless dreva je rsync IZPUSTIL postcss.config.mjs (prejšnje seje so ga imele) → brez @tailwindcss/postcss pipeline je Turbopack razreševal CSS uvoz z navadnim module resolution, ki ne pozna „style" export pogoja; popoln postopek: rsync MORA vključevati postcss.config.mjs (+ eslint.config.mjs), bun.lock (npm pot povzroči ENOENT .next/next-server.js.nft.json), pravi orgId v .vercel/project.json (team_KuVkEd4HM0StzdABN6HqzycD, dobljen prek API v9/projects), db/demo-template.db v db/ mapi; PACKAGE: tw-animate-css premaknjen devDependencies → dependencies (varnost za namestitve z obrezanimi dev paketi)
- PRODUKCIJA (terminai-eight.vercel.app): deploy OK; / 200, /api 200 (health: app/status/jezik), /api/services 200, /api/birthdays z novimi polji (Ana danes, Petra +3 z „Barvanje + prha" — priljubljena iz PRODUKCIJSKE baze), /api/loyalty (Nuša win-back) nedotaknjen ✓; kartica se izriše, tema LIGHT, 0 napak v konzoli
- USB: export-usb.sh (build, offline test 3456: 200 + API + 0 CDN) → dist-usb/TERMINAI 326 MB; POPRAVLJENA IZGUBLJENE DEMO FOTOGRAFIJE (custom.db jih ni imel — verjetno izgubljene ob prejšnjem demo resetu): preneseni 2 sličici iz demo-template.db („after balayage 6-34 + 7-31" vezana na Anin zadnji zaključen Barvanje termin + „reference šlosa" na stranko) — preverjeno z /api/photos in v samem zipu (2 fotos, 7 rojstnih dni); TERMINAI-USB.zip regeneriran (156 MB)
- DOKUMENTACIJA: README (razdelek Rojstni dnevi prepsan: osebna čestitka, nastavljiva ugodnost, oznake poslano, gumb Naroči), usb-template/ZA-TEBE.txt NOVOSTI 39; git commit + push
- SANDBOX: dev server dvakrat tiho umrl v sesiji → vzdrževalni restart; po USB buildu rm -rf .next + čist zagon (znan vzorec)

Stage Summary:
- Motor zvestobe P4 je ZAKLJUČEN (A win-back + B rojstni dnevi + C rebooking): rojstnodnevna čestitka je zdaj osebna (priljubljena storitev, podpis salona), ugodnost je pod nadzorom lastnika (privzeto brez obljub), dvojne čestitke nemogoče (letne oznake), en klik naroči „rojstnodnevni obisk" na pravi dan
- DEPLOY POSTOPEK DOKUMENTIRAN v worklogu: manjkajoči postcss.config.mjs je bil vzrok propadov — popoln rsync seznam je zdaj znan
- Popravljena tudi tiha izguba demo fotografij na USB (2 sličici vrnjeni v custom.db)
- Naslednji možni koraki: pilot pri salonu (USB + letak + demo vodik), GitHub Actions za samodejni deploy, online faza (prava baza + WhatsApp Business API)
